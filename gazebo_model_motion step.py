...
class BoxMotionController:
    ...
    def __init__(self):
        ...
        # Box parameters
        self.amplitude = {box_name: params['dim'] / 2 for box_name, params in self.box_params.items()}
        self.frequency = {'box_fl': 0.0, 'box_fr': 0.2, 'box_rl': 0.0, 'box_rr': 0.2}
        self.phase_offsets = {'box_fl': 0.0, 'box_fr': 0.0, 'box_rl': 0.0 , 'box_rr': 0.0}
            ...
            if self.start_motion and not self.stop_motion:
                ...
                for box_name, params in self.box_params.items():
                    ...
                    if self.time < time_to_first_min:
                        ...
                        position = - self.amplitude[box_name] * math.cos(2 * math.pi * self.frequency[box_name] * (self.time)) - (position_offset)
